...
function doPopout(e) {
    ...
    if ($('#objectiveExtensionPopout').length === 0) {
        ...
        newElement.attr('id', 'objectiveExtensionPopout')
            ...
            .addClass('draggable')
            .empty();
        const prevObjectiveBoxContents = $('#objective_contents').val(); //copy summary box before emptying
        originalElement.empty();
        originalElement.html('<div class="flex-container alignitemscenter justifyCenter wide100p"><small>Currently popped out</small></div>');
        newElement.append(controlBarHtml).append(originalHTMLClone);
        $('body').append(newElement);
        $('#objectiveExtensionDrawerContents').addClass('scrollableInnerFull');
        // setMemoryContext(prevSummaryBoxContents, false); //paste prev summary box contents into popout box
        // setupListeners();
        loadSettings();
        loadMovingUIState();

        $('#objectiveExtensionPopout').fadeIn(animation_duration);
        dragElement(newElement);

        //setup listener for close button to restore extensions menu
        $('#objectiveExtensionPopoutClose').off('click').on('click', function () {
            $('#objectiveExtensionDrawerContents').removeClass('scrollableInnerFull');
...
jQuery(() => {
    ...
    $('#objectiveExtensionPopoutButton').off('click').on('click', function (e) {
        doPopout(e);
        e.stopPropagation();
    });
    loadSettings();
